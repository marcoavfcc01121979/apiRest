...
class ReserveController{ 
  ...
  async store(req, res) {
    ...
    if(house.status !== true) {
      return res.status(400).json({ error: 'Solicidação indisponivel.' }); 
    }
  ...
  async destroy(req, res) {
    ...
    await Reserve.findByIdAndDelete({ _id: reserves_id })

    return res.send(`${reserves_id} deletado com sucesso.`)
  }
}
